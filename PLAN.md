...
- Convert ages to whole months from the start of the projection: `retirement_month` = round((`retirement_age` − `current_age`) × 12), `total_months` = (`end_age` − `current_age`) × 12 (whole number, since `end_age` is a whole number and `current_age` is monthly-precision).
- Loop over month index m = 1, 2, 3, ... up to `total_months`. Each block of 12 consecutive months (m=1–12, m=13–24, etc.) corresponds to one simulated year and uses one historical year's return: for month m, the historical year is `S + floor((m − 1) / 12)`. If that historical year is greater than `last_data_year`, this simulation stops here — it has no data to continue, and does not contribute to month m or any later month.
  1. **Convert that block's annual return to a monthly rate:** `monthly_rate = (1 + return_pct/100)^(1/12) − 1` (the geometric conversion — compounding this rate 12 times reproduces the actual annual return exactly, not a simple divide-by-12 approximation).
  2. **Determine this month's phase:** if m ≤ `retirement_month`, this month is pre-retirement (contribution); otherwise it's post-retirement (withdrawal). This is evaluated fresh every month, which is what allows retirement to fall mid-year.
  3. **If pre-retirement:** `monthly_contribution_posttax` = sum of post-tax accounts' `annual_contribution` ÷ 12; `monthly_contribution_pretax` = sum of pre-tax accounts' `annual_contribution` ÷ 12.
     **If post-retirement:** calculate the current year's total withdrawal need as `expected_expenses_in_retirement` inflated by `inflation_rate_pct`, compounded for `(m − 1) / 12` years (fractional years elapsed since the start of the projection — inflation compounds continuously, not in yearly steps), then divide by 12 for `monthly_withdrawal_need`. Whether post-tax-only or split applies is determined this month using the fractional age at this month (`current_age + (m − 1)/12`) compared to 59.5.
  ...
  5. If m is the last month of a block of 12 (i.e., `m % 12 == 0`) or the last month overall, record the combined (post-tax + pre-tax) balance as this simulation's result for that forward year (offset n = m / 12, rounded up).
...
**Step 5 — Warn on early pre-tax access.**
If any simulation triggered the pre-59.5 pre-tax fallback (Step 2, part 4), show a separate warning noting this happened, including the earliest age at which it occurred across all simulations (e.g. "In at least one historical scenario, pre-tax funds were accessed before age 59.5 because post-tax funds ran out, starting at age X"). This is distinct from the depletion warning and the data-coverage warning — it's flagging a real-world consideration (early-withdrawal penalties/tax treatment aren't modeled in v1), not an error.
