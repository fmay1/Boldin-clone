...
- Unknown/invalid `return_mode` value → validation error naming the valid modes (all three endpoints)
...
12. **Live tab expenditure support** — Add the same planned large expenditures UI (inline editable list with + / edit / delete) to the Live page's form. When a scenario is selected via dropdown, fetch and populate its expenditures. On "Update Projection", include the expenditures array in the POST body to `/api/projection/preview` so preview results reflect them. On "Save Changes to Scenario", sync the expenditure list (PUT existing, POST new, DELETE removed) alongside the scenario update.
13. **"Save as New Scenario" button** — In the Scenarios tab edit form, add a "Save as New Scenario" button (light blue background) next to the existing Update and Cancel buttons. When clicked, sends `POST /api/scenarios` with all edit form values plus `"copy_expenditures": true` and `"source_scenario_id": editingId`. Backend copies the scenario fields and all associated expenditures from the original into a fresh new record. On success, resets the form and refreshes the list.

14. **Future income** — New `scenario_incomes` table (one-to-many with scenarios): `id`, `scenario_id` (FK), `start_age` (decimal, monthly precision), `end_age` (decimal, monthly precision), `amount` (today's dollars), `inflation_adjusted` (boolean). Backend CRUD routes (`POST/PUT/DELETE /api/scenarios/<id>/incomes`) with no hard limit on count; GET `/api/scenarios` returns nested incomes array. Projection engine modified to add each income line during its active period (from `start_age` inclusive through `end_age` exclusive, evaluated monthly so fractional ages work correctly), with inflation applied if `inflation_adjusted` is true — same compounding logic as expenses but added to the balance instead of subtracted. Frontend: expandable list in scenario form — single line per income with `+` button to add more, inline editable start age/end age/amount/inflation toggle, delete icon. "Save as New Scenario" copies incomes alongside expenditures.
...
- Scenario input validation, previously copy-pasted across the create/update/preview endpoints, is consolidated in a single `validate_scenario()` function in app.py (the duplication had already drifted twice: the preview endpoint was missing the monthly-precision check and had no start>end year check). Unknown `return_mode` values are now rejected with a 400 naming the valid modes (previously create/update hit the SQLite CHECK constraint → 500, and preview fell through to a late "Unsupported return mode" error in the engine); missing core fields on create/update now return 400 instead of 500.
